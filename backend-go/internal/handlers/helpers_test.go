...
func TestRecreateCacheSetGet(t *testing.T) {
    ...
    // Set values
    app.SetRecreateNecessary("stack-a", true)
    app.SetRecreateNecessary("stack-b", false)

    cache = app.GetRecreateCache()
    if !cache["stack-a"] {
        t.Error("expected stack-a = true")
    }
    if cache["stack-b"] {
...
func TestGetRecreateCacheNilMap(t *testing.T) {
    t.Parallel()

    app := &App{} // RecreateCache is nil
    cache := app.GetRecreateCache()
    if cache == nil {
...
func TestSetRecreateNecessaryNilMap(t *testing.T) {
    t.Parallel()

    app := &App{} // RecreateCache is nil
    app.SetRecreateNecessary("test", true)

    cache := app.GetRecreateCache()
    if !cache["test"] {
        t.Error("expected test = true after setting on nil map")
    }
}
